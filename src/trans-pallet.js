...
export default class TransPallet extends Object3D {
  static get threedObjectLoader() {
    if (!TransPallet._threedObjectLoader) {
      TransPallet._threedObjectLoader = new Promise((resolve, reject) => {
        let objLoader = new THREE.OBJLoader(THREE.DefaultLoadingManager);
        let mtlLoader = new THREE.MTLLoader(THREE.DefaultLoadingManager);
        ...
        mtlLoader.load('TransPallet.mtl', (materials) => {
          materials.preload();
          objLoader.setMaterials(materials)
          materials.side = THREE.frontSide

          objLoader.load('TransPallet.obj', (obj) => {
            var extObj = obj
            // if (extObj && extObj.children && extObj.children.length > 0) {
            //   extObj = extObj.children[0];
            // }
...
export class TransPallet2D extends RectPath(Shape) {
  is3dish() {
    return true
  }
